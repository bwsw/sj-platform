...
class Executor(manager: OutputEnvironmentManager) extends OutputStreamingExecutor[Integer](manager) {
  ...
  private val column =
    serializer.deserialize[Map[String, Any]](manager.options)
      .getOrElse(Data.columnField, "value").asInstanceOf[String]
  ...
  override def onMessage(envelope: TStreamEnvelope[Integer]): List[Data] = {
    ...
    envelope.data.toList.map(i => new Data(column, i))
  }

  override def getOutputEntity: Entity[(PreparedStatement, Int) => Unit] = {
    new JdbcEntityBuilder()
      .field(new IntegerField(column))
      .build()
  }
}
