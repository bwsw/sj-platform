...
class RegularInstanceValidatorTestSuit extends FlatSpec with Matchers with InstanceValidatorMocks {
  private val instanceValidator = new RegularInstanceValidator()(injector)

  it should "validate() method returns non empty set of errors if instance and specification is blank" in {
    //arrange
    val instance = new RegularInstanceWithDefaultFieldsMock().instance
...
class RegularInstanceWithDefaultFieldsMock() extends MockitoSugar {
  val instance = mock[RegularInstance]
