...
/**
  * Provides methods for running [[BatchReaderBenchmark]] and writing a result into a file
  *
  * @param benchmark benchmark
  * @param config    config for benchmark
  * @author Pavel Tomskikh
  */
class BatchReaderBenchmarkRunner(benchmark: BatchReaderBenchmark,
                                 config: BatchReaderBenchmarkConfig)
  extends ReaderBenchmarkRunner(config) {

  def run(): Seq[BatchReaderBenchmarkResult] = {
    benchmark.warmUp()
