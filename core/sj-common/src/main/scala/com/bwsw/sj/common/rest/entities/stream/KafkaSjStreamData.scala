    ...
    Option(this.service) match {
      ...
      case Some(x) =>
        ...
        serviceObj match {
          ...
          case Some(service) =>
            if (service.serviceType != ServiceConstants.kafkaServiceType) {
              errors += s"Service for ${StreamConstants.kafkaStreamType} stream " +
                s"must be of '${ServiceConstants.kafkaServiceType}' type ('${service.serviceType}' is given instead)"
            }
        }
    }
